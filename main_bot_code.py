...
def color_to_rgb(color):
    ...
    correct = (color[0] == '(' and color[-1] == ')' and
               len(color[1:-1].split(', ')) == 3 and
               color[1:-1].split(', ')[0].isdigit() and
               0 <= int(color[1:-1].split(', ')[0]) <= 255 and
               color[1:-1].split(', ')[1].isdigit() and
               0 <= int(color[1:-1].split(', ')[1]) <= 255 and
               color[1:-1].split(', ')[2].isdigit() and
               0 <= int(color[1:-1].split(', ')[2]) <= 255)
    if correct:
