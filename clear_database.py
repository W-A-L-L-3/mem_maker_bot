...
def clear_all_users_files(del_data=False):
    ...
    ВНИМАНИЕ! Если удалить этот файл, и после боту напишет пользователь,
    который ранее писал ему "/start" (т. е. у этого пользователя уже был файл
    data, который в последствии был удалён), и сообщение от пользователя будет
    не командой "/start", то произойдёт ошибка. НАДО ЭТО ИСПРАВИТЬ!
    """
